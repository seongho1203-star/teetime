...
/* ── 4-1. 물음표가 갈래를 가른다 ───────────────────────────────
   사용자 요청 — `응? 엥? 뭐? → 응?·엥?·뭐?·어쩌라고?`. 한 글자 말이라
   물음표 없이는 못 넣는다(`응`은 `응원`에도 걸린다). */
console.log('\n── 물음표 ──');
for (const w of ['응?', '엥?', '뭐?', '어쩌라고']) {
    ok(['응?', '엥?', '뭐?', '어쩌라고?'].every(l => has(w, l)), `\`${w}\` → 응?·엥?·뭐?·어쩌라고?`);
}
ok(has('응？', '응?'), '전각 `？`도 같다');
ok(!has('응원합니다', '응?'), '`응원`에는 안 뜬다');
ok(!has('뭐해요', '응?'), '`뭐해요`에는 안 뜬다(그쪽은 뭐해 꼭지다)');
ok(has('뭐해요', '뭐해요?'), '`뭐해요` → `뭐해요?`는 그대로 걸린다');
ok(has('헉 대박', '헉!'), '`헉` → `헉!`');
ok(has('헉 대박', '헉!?'), '`헉` → `헉!?`(물음표 열쇠도 표에 적혀 있다)');
