 ...
 *
 * **지금은 22판이다** — 사진·이모티콘·눌리는 카드(2판) · 인용·반응 알약·
 * `여기까지 읽으셨습니다` 줄(3판) · 손짓(4판) · 굴리기 얽힘(5판)까지
 * 앱이 맡는다. 그 아래 판을 든 폰은 못 그리거나(사진이 글자로 보인다)
 * 못 움직여서(길게 눌러도 창이 안 뜬다), **같은 수로 웹 목록으로 되돌린다.**
 *
 * **20 → 21 → 22가 그 본보기다** — 20판은 얼굴·인용·밀기가 안 눌리고
 * `최근 대화로` 줄이 가려 안 보였고, **21판은 이모티콘이 한 장도 안 떠
 * 말풍선 자리가 통째로 비었다**(앱 안에 든 그림을 인터넷에서 받으려
 * 했다). 수를 올려 두면 그 판을 든 폰은 **웹만 밀어도 그날로** 웹
 * 목록으로 돌아오고, 새 앱을 받으면 저절로 앱 목록으로 돌아온다.
 */
export function canNativeList(): boolean {
    return ncLog.ready === true && ncLog.v >= 22 && listOn();
}
