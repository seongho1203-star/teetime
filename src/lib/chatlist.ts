 ...
 *
 * 카톡의 `삭제된 메시지입니다`와 같은 자리다 — **말풍선을 벗기고 흐린
 * 한 줄로만** 그린다(사용자가 고른 모양이다). 예전에는 말풍선 안에
 * `운영진이 가린 메시지입니다`로 넣었는데, 그러면 덮어 둔 글이 오히려
 * 여느 말보다 도드라진다.
 *
...
export type ListRow = {
     ...
     * 흐린 한 줄**(지금은 운영진이 가린 글 하나다 — `note`에 적힌 말만
     * 그린다. 덮어 둔 내용이 새면 안 되므로 원문은 아예 안 넘긴다).
     */
    ...
    date?: string;
    /**
     * 말풍선 대신 **흐린 한 줄로만** 적을 말 — 지금은 가린 글
     * (`HIDDEN_LINE`) 하나다. 이 값이 있으면 앱도 웹처럼 말풍선을 벗긴다.
     */
    note?: string;
    /**
