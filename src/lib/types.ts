...
/**
 * 정산의 `입금 은행`에서 고르는 목록(사용자 요청 — `정산에서 입금은행을
 * 누르면 은행목록이 나오고 거기에서 선택할수있게해줘`).
 *
 * **손으로 치던 칸이었다.** 계좌번호가 한 글자만 틀려도 돈이 엉뚱한 데로
 * 가는 자리라 은행 이름까지 오타가 날 이유가 없고, 무엇보다
 * **`토스로 보내기`가 이 글자로 은행을 찾는다**(`tossUrl` in
 * Settlement.tsx) — `국민은행`처럼 정식 이름으로 골라 두면 그쪽이 끝의
 * `은행`만 떼어 넘기므로 늘 맞는다. 직접 치면 `국민 은행`·`kb`처럼
 * 제각각이 되어 토스가 못 알아본다.
 *
 * **차례는 우리 모임 기준이다** — 많이 쓰는 시중은행 · 인터넷은행 ·
 * 그다음 지역은행인데 **`광주은행`을 앞에 세웠다**(회원 대부분이
 * 광주·전남이다). 가나다순으로 되돌리지 말 것.
 *
 * **`직접 입력`을 빼지 말 것** — 증권사 계좌나 여기 없는 곳을 쓰는 사람이
 * 있는데, 목록뿐이면 그 사람은 정산을 아예 못 만든다. 고르면 칸이 하나
 * 열린다(`Settlement.tsx`의 `OTHER`).
 */
export const BANKS = [
    '국민은행', '신한은행', '우리은행', '하나은행', '농협은행', '기업은행',
    '광주은행', '카카오뱅크', '토스뱅크', '케이뱅크',
    '새마을금고', '신협', '우체국', '수협은행',
    '부산은행', '대구은행', '경남은행', '전북은행', '제주은행',
    'SC제일은행', '씨티은행', '산업은행',
] as const;
...
/**
 * **카카오 선물하기로 가는 문**(전체화면 프로필의 `🎁 선물하기`).
 *
 * **우리 앱이 선물을 보내는 것이 아니다 — 카카오 페이지를 열어 줄 뿐이다.**
 * 카카오가 밖에 열어 둔 것은 로그인·공유·지도·내비·페이뿐이고, 선물을
 * 대신 보내 주는 API는 **사업자 전용 `선물하기 for Biz`** 하나뿐이라
 * 우리 같은 모임 앱에는 길이 없다. 그래서 정산의 `토스로 보내기`와
 * **완전히 같은 짜임**이다 — 주소 하나를 여는 지름길이지 대체가 아니다.
 *
 * - **받는 사람을 미리 못 고른다.** 카카오는 주소에 상대를 안 받는다 —
 *   누구에게 보낼지는 카카오톡 안에서 고른다. 그래서 **누구 프로필에서
 *   눌러도 같은 화면이 뜬다**(눌러 놓고 상대를 또 고르게 되므로, 이
 *   단추가 하는 일은 `즐겨찾기 한 칸`이 전부다).
 * - **카카오톡 친구가 아니면 못 보낸다.** 우리 앱에는 친구 목록을 볼
 *   권한이 없다.
 * - **분홍을 쓰지 말 것** — 이 화면에서 '지금 눌러야 할 것'은 보내기
 *   단추 하나다(`토스로 보내기`가 분홍이 아닌 것과 같은 잣대다).
 *
 * **앱에도 같은 값이 박혀 있다**(`NativeChatDrawer.swift`의 `ChatProfile`) —
 * 한쪽만 고치지 말 것.
 *
 * **웹에서는 이 주소가 전부다 — 카카오톡 앱을 여는 길이 없다**(사용자
 * 질문 — `사이트가 아니고 카톡 앱이 열려서 선물하기는 안 되나?`).
 * 사파리에서 앱이 열리느냐는 **카카오가 제 서버에 등록해 둔 유니버설
 * 링크**에 달려 있어 우리가 정할 수가 없고, 스킴(`kakaotalk://…`)을
 * 주소창에 밀어 넣는 길은 **앱이 없는 폰에서 `주소가 올바르지 않습니다`
 * 창만 띄운다**(홈 화면 앱에서는 돌아올 길도 없다).
 * **앱은 갈래가 하나 더 있다** — `ChatProfile.giftTapped`가 스킴을 먼저
 * 열어 보고 **정말 안 열렸을 때만** 이 주소로 물러난다.
 */
export const GIFT_URL = 'https://gift.kakao.com/';
...
export type Message = {
    ...
    round_id?: string | null;
    /**
     * 어느 공지 이야기인가. 라운드와 **같은 구실**이다 — 공지 상세의
     * `📣 대화방에 공유`로 올린 줄에 붙고, 눌러서 그 공지로 들어간다.
     * 없는 저장소에서는 `undefined`라 예전처럼 가운데 한 줄이다.
     */
    post_id?: string | null;
    /**
     ...
     * 화면은 글·사진·이모티콘 대신 `가려진 메시지입니다`를 그린다
     * (말풍선 없이 흐린 한 줄 — `HIDDEN_LINE` in lib/chatlist.ts).
    ...
    /**
     * **없앤 `방 공지`가 남긴 칸이다**(사용자 요청 — `채팅방 공지는 삭제해 줘`).
     * 화면은 더는 안 읽고 안 쓴다 — `rounds.title`·`opens_at`과 같이
     * **기록으로만 남겨 둔 것**이라 지우는 SQL도 없다. 되살리지 말 것.
     */
