 ...
 *
 * **긴 변 1600px · JPEG 82%다.** 한 장 300KB쯤이다.
 *
 * **2560px으로 올렸다가 되돌렸다 — 사진이 아예 안 올라갔다**(사용자 제보 —
 * `사진 크기를 키운 후로 안돼`). 화질만 보고 키웠다가 올리는 길이 막힌
 * 것이라, **되돌리는 것이 먼저였다.** 어디서 막히는지는 아래
 * `sendPhoto`의 오류 문구로 가린 뒤에 다시 올린다.
 * **바꿀 때는 사용자에게 물을 것** — 무료 저장 공간이 1GB라 여기가
 * 곧 '사진을 몇 장 올릴 수 있는가'이고, **키우면 안 올라갈 수도 있다.**
 *
