 ...
 *
 * **`mv*`는 묶음이 달라도 값이 넷 다 같아야 한다** — 새로 넣을 때도,
 * 사용자가 만들어 보내 준 것을 받을 때도 여기에 맞춘다:
 *
 * 1. **256px 정사각.** 앱은 프레임을 통째로 풀어 담으므로(`ChatList.swift`의
 *    `Shot.cost` = 가로×세로×4×프레임 · `cache.totalCostLimit` 48MB)
 *    320px이면 한 탭만으로 캐시를 거의 다 먹어 **서랍이 늦게 뜬다.**
 *    **파일 크기가 아니라 푸는 값을 셈해 볼 것.**
 * 2. **한 바퀴가 1200ms**(`한 장 = 1200 ÷ 프레임 수` — 12장이면 100ms,
 *    8장이면 150ms). `몇 fps인가`가 아니라 **한 바퀴가 몇 ms인가**로 견준다.
 * 3. **`loop=3`**(무한 반복이 아니다). 늘 켜져 있는 그리기 비용이
 *    안드로이드에서 화면을 끊기게 한다.
 * 4. **`<id>.png`(가운데 프레임)를 같이 둔다.** 서랍이 처음 열릴 때 먼저
 *    까는 멈춘 그림이자 옛 판을 든 폰의 예비 길이다(64색 · 7~13KB).
 *
 * 2·3은 **ANMF·ANIM 칸의 몇 바이트만 고치면 되므로 다시 굽지 않는다**
 * (화질이 한 톨도 안 상한다). 1만 다시 굽는다 — webp 품질 80 · method 4 ·
 * LANCZOS. **화질은 보라 바탕(`--chat-bg`)에 얹어 놓고 잴 것**: 투명한
 * 자리의 RGB까지 세면 헛값이 나온다(webp는 알파 0인 픽셀의 색을 안 지킨다).
 */
 ...
 * 묶음 목록.
 *
 * 아홉 묶음이다 — **프렌즈 쉰넷**(사용자가 만들어 보내 준 움짤 · 아래 참고),
 * **펭귄 움짤 서른**(검은 바탕 영상에서 잘랐다 — 아래 참고),
 * 골프공(앱 아이콘과 같은 캐릭터) 스물일곱, 골퍼 스물넷, 골프친구 스물넷,
 * 펭귄골프 스물둘, 펭귄 스물여덟, 고양이 스물넷, 이모지 열여덟.
 * **차례는 원본에 나온 차례 그대로다.**
 *
 * **움직이는 묶음이 앞에 선다.** 서랍을 열면 먼저 보이는 자리가 거기다.
 *
 * **사용자가 보고 지우라고 한 묶음이 넷이다 — 되살리지 말 것**
 * (뒤엣둘은 사용자가 다시 만들어 올려 `🐾 프렌즈`로 돌아왔다)**.**
 * 그림 파일까지 함께 지웠다:
 *
 * - `🏌️ 골프공 움짤` 스무 장 — `골프공 움짤은 별로네. 삭제해줘`.
 * - `✨ 움직임` 열여덟 · `👦 소년 움짤` 마흔넷 —
 *   `움직임과 소년움짤은 삭제해줘. 퀄리티도 별로고 지금 속도도 너무 느려`.
 *   그 둘이 13MB였고(19 → 6.2MB) 첫 묶음이 통째로 움직이는 판이라 서랍을
 *   열 때 제일 무거운 자리였다.
 * - **`😊 프렌즈 감정` 열둘 · `💬 프렌즈 일상` 열둘** —
 *   `프렌즈 감성하고 일상 삭제해줘. 다시 올릴게`.
 *   사용자가 만들어 보내 준 묶음이었다(`..._Animated_Sticker_Pack_v1.zip` ·
 *   `..._Daily_Sticker_Pack_v2.zip`). **말 그대로 다시 올라왔다** —
 *   지금 맨 앞의 `🐾 프렌즈` 쉰넷이 그것이고, 그 스물넷은 **옛 id를
 *   그대로 물려받았다**(아래 참고).
 */
export const STICKER_GROUPS: StickerGroup[] = [
    /* 사용자가 만들어 보내 준 묶음이다(`..._Complete_54_256px_12f_v1.zip`).
       **받은 그대로 규칙에 맞았다** — 256px · 12장 · 한 장 100ms(한 바퀴
       1200ms)라, 위 `mv*` 넷 가운데 **고친 것은 `loop` 두 바이트 하나뿐**이고
       (0 → 3) `<id>.png`(일곱째 장)를 함께 만들었다. **다시 굽지 않았다.**

       **앞 스물넷은 지운 `😊 프렌즈 감정`·`💬 프렌즈 일상`과 같은 그림이라
       옛 id를 그대로 쓴다**(`mvf*`·`mvd*`) — 그래야 그사이 그걸로 보낸
       예전 글이 도로 살아난다. 이름·캐릭터가 열두 개씩 하나도 안 어긋나
       짝이 분명했다. 나머지 서른은 새것이라 `mvk*`다.

       **한 묶음으로 뒀다.** 보내 준 글에는 카테고리가 일곱으로 갈려 있지만
       (감정·일상·유행어·골프·출퇴근·모임·선물) **한 세트라 한 탭이 맞고**,
       일곱을 그대로 만들면 탭이 열다섯이 되어 탭 줄만 화면을 먹는다.
       쉰넷은 백 장 아래라 한 탭으로 훑을 수 있다. 차례는 보내 준 차례다.

       **이 묶음만으로 9.3MB다**(한 장 141~224KB — 펭귄 움짤의 두 배쯤).
       앱 묶음에 그대로 들어가는 값이라 사용자에게 알리고 넣었다. */
    g('kkf54', '🐾', '프렌즈',
      a('mvflaugh', '폭소'), a('mvfcry', '엉엉'), a('mvfangry', '화남'),
      a('mvfshout', '버럭'), a('mvfsulk', '삐짐'), a('mvfshock', '깜짝'),
      a('mvfthanks', '감사'), a('mvfsorry', '미안'), a('mvfparty', '축하'),
      a('mvfcheer', '응원'), a('mvflove', '사랑'), a('mvfhuh', '황당'),
      a('mvdhello', '안녕'), a('mvdbye', '잘가'), a('mvdok', '좋아'),
      a('mvdno', '안돼'), a('mvdwait', '잠깐'), a('mvdgo', '가자'),
      a('mvdnight', '잘자'), a('mvdmeal', '밥 먹자'), a('mvdbusy', '바빠'),
      a('mvdwhat', '뭐해?'), a('mvdcall', '전화해'), a('mvdcheers', '짠!'),
      a('mvkshrug', '어쩌라고'), a('mvktongue', '메롱'), a('mvkboo', '깜짝이야'),
      a('mvkrun', '튀어'), a('mvkhush', '쉿'), a('mvksigh', '아이고'),
      a('mvkdriver', '나이스샷'), a('mvkgood', '굿샷'), a('mvkputt', '나이스 퍼트'),
      a('mvkbunker', '벙커 탈출'), a('mvkob', '오비'), a('mvkbirdie', '버디'),
      a('mvkgowork', '출근합니다'), a('mvksubway', '출근길'), a('mvkcoffee', '커피 충전'),
      a('mvkpile', '일 폭탄'), a('mvknight', '야근 중'), a('mvkoff', '퇴근이다'),
      a('mvkcal', '날짜 잡자'), a('mvkonway', '가는 중'), a('mvkhere', '도착'),
      a('mvklate', '늦어요'), a('mvkwhere', '어디야'), a('mvkbook', '예약 완료'),
      a('mvkgift', '선물이야'), a('mvkthx', '고마워'), a('mvkcake', '생일 축하해'),
      a('mvkparty', '축하해'), a('mvkflower', '꽃다발'), a('mvkwish', '소원 빌어')),

    /* **검은 바탕에 격자로 깔린 영상**에서 잘랐다. 체커 바탕 것들과 달리
       바탕이 순수한 검정이라, **밝기로 딴 것과 rembg로 딴 것을 겹쳐**
       알파를 만들었다 — 밝기는 하트·글자·반짝임까지 픽셀 단위로 정확하고,
       rembg는 밝기로는 바탕과 구별이 안 되는 **어두운 것**(선글라스·모자
       그늘·머리카락)을 맡는다. 10fps · 열두 장 · `loop=3`이고, 한 장이
       120~250KB다.

       **`💙 펭귄`과 같은 캐릭터지만 묶음을 따로 두었다** — 그쪽은 정지 그림
       시트이고 이건 움직이는 판이라 서로 다른 세트다. 한 탭에 섞으면 같은
       말이 두 번씩 들어가 쉰 장 넘는 탭이 된다. */
    /* 노트북 앞에 앉은 것(`mvp41`)에 **사과 상표가 있어 지웠다** —
       `pnwork`·`ctwork` 때와 같은 자리이고, 사용자가 그때 `로고만 지워줘`로
       정한 방식 그대로다. 움직이는 것이라 열두 장을 다 메웠다. */
    g('mvpeng', '❄️', '펭귄 움짤',
      a('mvp11', '반가워!'), a('mvp12', '하트'), a('mvp13', '빵터짐'),
      a('mvp14', '좋아요'), a('mvp15', '부끄'), a('mvp16', '화이팅!'),
      a('mvp21', '꾸벅'), a('mvp22', '커피 한 잔'), a('mvp23', '잘 자'),
      a('mvp24', '빼꼼'), a('mvp25', '엉엉'), a('mvp26', '헉!?'),
      a('mvp31', '꽃다발'), a('mvp32', '멋져'), a('mvp33', '하트 눈'),
      a('mvp34', '윙크'), a('mvp35', '축하해'), a('mvp36', '개구리'),
      a('mvp41', '일하는 중'), a('mvp42', '감사합니다'), a('mvp43', '한잔해요'),
      a('mvp44', '신난다'), a('mvp45', '화났어'), a('mvp46', '달려!'),
      a('mvp51', '포근'), a('mvp52', '부탁해'), a('mvp53', '뻗음'),
      a('mvp54', '비 와요'), a('mvp55', '신나는 음악'), a('mvp56', '안녕~')),
