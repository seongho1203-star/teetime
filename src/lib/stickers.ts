 ...
 *   열 때 제일 무거운 자리였다.
 * - **프렌즈 — 세 번 들어왔다 나갔다.** `😊 프렌즈 감정` 열둘 ·
 *   `💬 프렌즈 일상` 열둘이 먼저였고(`프렌즈 감성하고 일상 삭제해줘.
 *   다시 올릴게`), 그 말대로 `🐾 프렌즈` 쉰넷(`..._Complete_54_256px_12f_v1.zip`)
 *   이 다시 올라왔다가 **그것도 지웠다**(`프랜즈이모티콘삭제해줘`).
 *   셋 다 사용자가 만들어 보내 준 움짤이다 — **또 올라올 수 있으므로**
 *   맞추는 다섯 걸음(`mv` id · `loop=3` · 예비 png · 한 바퀴 1200ms ·
 *   256px)은 `ANIM_PREFIX`와 CLAUDE.md에 그대로 남겨 두었다.
 *   쉰넷 가운데 앞 스물넷은 옛 id(`mvf*`·`mvd*`)를 물려받았었다 —
...
export const STICKER_GROUPS: StickerGroup[] = [
    ...
    /* **사용자가 만들어 보내 준 흰곰**(`reaction_stickers_webp.zip` 넷 ·
       `daily_stickers_6pack_256.zip` 여섯 · 대화창에 붙여 준 `ㅋㅋㅋ` 한 장).
       받은 것은 256px 정지 webp(배경 투명)였고 우리 규칙대로 **64색 팔레트
       PNG**로 바꿔 넣었다(장당 5~7KB · 테두리 잡티 0). 움직이는 판이
       아니라 id가 `mv`로 시작하지 않는다(`wb` = white bear).
       **펭귄 움짤 바로 뒤에 둔 것은 일부러다** — 사용자가 손수 만든 세트라
       서랍을 열면 곧바로 보이는 자리에 두었다. */
    g('wbear', '🐻‍❄️', '흰곰',
