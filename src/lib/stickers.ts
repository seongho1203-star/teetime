 ...
 * 묶음 목록.
 *
 * 열 묶음이다 — **프렌즈 감정 열둘 · 프렌즈 일상 열둘**(사용자가 만들어 준
 * 묶음이다 — 아래 참고), **펭귄 움짤 서른**(검은 바탕 영상에서 잘랐다 — 아래 참고),
 ...
 *
 * **사용자가 보고 지우라고 한 묶음이 셋이다 — 되살리지 말 것.**
 * 그림 파일까지 함께 지웠다:
 *
 ...
 *   열 때 제일 무거운 자리였다. **그 자리를 아래 `😊 프렌즈 감정`이 메웠다.**
 */
export const STICKER_GROUPS: StickerGroup[] = [
    /* **사용자가 만들어 보내 준 묶음이다**(`JTFAG_Kkakung_Animated_Sticker_Pack_v1.zip`
       — 딸려 온 `CLAUDE_INSTRUCTIONS.md`대로 넣었다). 동물 여섯이 두 장씩이고
       글자가 없다. 받은 것은 320px · 8프레임이었고, 지금은
       **256px · 8프레임 · 한 장 60~168KB**다(아래 5번).

       **받은 대로 넣지 않고 우리 규칙에 맞춘 것이 다섯이다:**

       1. **id를 `mv`로 시작하게 바꿨다**(`kkakung_laugh_dog` → `mvflaugh`).
          `stickerSrc()`가 머리글자로 확장자를 정하므로, 안 바꾸면 `.png`를
          찾아 404가 난다(`ANIM_PREFIX`).
       2. **반복을 무한(`loop=0`)에서 `loop=3`으로 바꿨다.** 늘 켜져 있는
          그리기 비용이 안드로이드에서 화면을 끊기게 한다 — **ANIM 칸의 두
          바이트만 고쳤으므로 다시 굽지 않았고 화질도 그대로다.**
       3. **`<id>.png`(가운데 프레임)를 같이 만들었다** — 서랍이 처음 열릴
          때 먼저 까는 멈춘 그림이자 옛 판을 든 폰의 예비 길이다. 64색으로
          줄여 한 장 7~13KB다.
       4. **한 장을 120 → 150ms로 늘려 한 바퀴를 1200ms로 맞췄다**
          (사용자 제보 — `이모티콘 펭귄움짤정도를 원하는데 이번에 내가 올린
          건 속도가 너무 빠르네`). **`❄️ 펭귄 움짤`이 12장 × 100ms = 1200ms라
          그 값에 맞춘 것이다.** 이쪽은 8장뿐이라 한 걸음이 크고 한 바퀴가
          960ms로 짧아 빠르게 느껴졌다 — **프레임을 늘릴 길은 없으므로
          (원본 영상이 없다) 한 장을 쥐고 있는 시간을 늘렸다.**
          여기도 ANMF 칸의 세 바이트만 고쳤다(다시 굽지 않았다).
       5. **320 → 256px로 줄였다**(사용자 제보 — `이모티콘눌렀을때도 엄청
          늦게뜨고`). **파일 크기가 아니라 푸는 값이 문제였다** — 앱은 프레임을
          통째로 풀어 담는데(`ChatList.swift`의 `Shot.cost` = 가로×세로×4×프레임 ·
          `cache.totalCostLimit` 48MB) 320px 여덟 장이면 한 장에 3.3MB라
          **첫 탭 열둘만으로 39MB**였다. 256px면 한 장 2.1MB라 24MB로 떨어진다
          (파일도 3,459 → 2,921KB). **`mv*`는 전부 256px이라는 규칙에도 맞는다.**
          여기만 **다시 구워야 했다**(webp 품질 80 · method 4 — 펭귄 움짤과
          같은 값이다). 보라 바탕에 얹어 재니 39.4dB이라 눈에 띄는 손해는 없다.

       **그래서 `mv*` 움짤은 묶음이 달라도 256px이고 한 바퀴가 다 1200ms다** —
       새로 넣을 때도 그 값에 맞출 것(`한 장 = 1200 ÷ 프레임 수`).

       **자리도 `public/stickers/`다** — 보내 준 글에는 `/assets/stickers/`로
       적혀 있었지만, 우리는 주소를 `stickerSrc()`가 만들고 서비스워커가
       `/stickers/` 아래만 캐시에 남긴다(`STICKER_CACHE`). 옮기면 그 둘이 함께
       깨진다. 글에 남는 값이 `sticker:<id>` 하나뿐인 것은 보내 준 글의
       `stickerId만 저장`과 같은 뜻이다. */
    g('kkf', '😊', '프렌즈 감정',
    ...
    /* 같은 사람이 보내 준 2차 묶음이다(`..._Daily_Sticker_Pack_v2.zip`).
       **같은 캐릭터 같은 그림체인데 탭을 따로 둔 것은 보내 준 글이 그렇게
       정했기 때문이다** — `선택창에 감정과 일상 카테고리 탭을 추가하고
       이번 12개는 일상에 표시합니다`. 세트로 가르는 우리 규칙과도 어긋나지
       않는다(1차·2차가 서로 다른 묶음이다).
       1차와 똑같이 **id를 `mv`로 바꾸고 · `loop=3`으로 고치고 · `<id>.png`를
       같이 만들고 · 한 장을 150ms로 늘리고 · 256px로 줄였다**(위 다섯 참고).
       그 다섯을 뺀 그림 자체는 받은 그대로다. */
    g('kkfd', '💬', '프렌즈 일상',
