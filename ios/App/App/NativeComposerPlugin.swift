...
public class NativeComposerPlugin: CAPInstancePlugin, CAPBridgedPlugin, ComposerBarDelegate,
    ...
    ///
    ///   - 24판 — **앱 목록을 눌러도 키보드가 내려간다.** 셀의 탭이 목록의
    ///        `키보드 내리기` 탭을 막고 있었다(둘 다 탭이라 깊이 있는 쪽이
    ///        이긴다) — 사용자 제보 `채팅창을 터치하면 키보드가 내려가지않아`.
    ///        셀 탭에 `SwipeGuard`를 달아 나란히 서게 했다.
    ...
    ///        주는 **남은 시간**만큼)이 그 몫이다.
    ///
    ///   - 26판 — **줄 위 자리를 웹이 알려 준다**(`ChatRow.top`). 예전에는
    ///        모든 줄에 `4`를 박아 두었는데 웹은 **다른 사람 사이 10px ·
    ///        같은 사람 잇따라 2px**이라, 줄마다 6px씩 어긋나 **아래로
    ///        갈수록 쌓였다.** 길게 누르는 창이 뜨면 앱 목록을 감추고 웹
    ///        목록으로 바꿔치기하는데 그때 그 어긋남이 통째로 드러난다
    ///        (사용자 제보 · 사진 두 장 — `팝업이 있을때와 없을때 프로필이나
    ...
    ///
    /// 31판 — **눌리는 카드에 머리 배지가 선다**(`ListRow.icon` ·
    ///        `ChatList.swift`의 `cardSymbol`). 라운드·투표·공지 카드가
    ///        셋 다 똑같이 생겨 목록을 훑을 때 글을 읽어야 무엇인지
    ///        알았다(사용자 요청 — `채팅 창에 라운드 뭐 공지 투표 이런 게
    ///        공유될 때 나오는 창을 예쁘게 바꿔줘`).
    ///        **`canNativeList()`의 문은 안 올렸다** — 30판 앱은 모르는 칸을
    ///        그냥 흘려 배지 없는 예전 카드를 그릴 뿐이라 깨질 자리가 없다.
    ...
    ///
    ///   - **33판** 눌리는 카드가 잔디빛으로 물들고 배지가 꽉 찬다
    ///        (`cardTint`·`cardBadge` · 제목이 먼저 오고 `○○님이 …했습니다`가
    ///        맨 아래로 — 웹의 `.chat-result`와 같은 차례).
